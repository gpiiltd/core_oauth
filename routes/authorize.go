...
func Authorize(r render.Render, res http.ResponseWriter, req *http.Request) {
	...
	fmt.Println("parsed params:client_id:" +
		client_id + ",response_type:" + response_type + ",redirect_uri:" + redirect_uri + ",scope:" + scope)
	...
	if response_type != "code" {
		...
		fmt.Println("error response_type:" + response_type)
		r.JSON(401, result)
		return
	}
	...
	if client == nil {
		...
		fmt.Println("client id not exist:" + client_id)
		r.JSON(401, result)
		return
	}
	...
	if redirect_uri != client.RedirectUri {
		...
		fmt.Println("redirect_uri not match, request:" + client.RedirectUri + ", exist:" + redirect_uri)
		r.JSON(401, result)
		return
	}

	redirectToLogin(r, client_id, redirect_uri)

}

func redirectToLogin(r render.Render, clientid string, clienturl string) {
	redirectUrl := util.Server.SSO_Login + "?clientcallback=" + clienturl + "&service="
	...
	fmt.Println("redirect to target:" + redirectUrl + encodedParams)
